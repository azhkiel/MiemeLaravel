    ...
    <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <div class="flex items-center space-x-4">
            ...
            <a href="{{ route('customer.chart') }}" class="relative p-2 rounded-full hover:bg-gray-100 transition-colors duration-200 group">
                ...
                <span class="cart-count absolute -top-1 -right-1 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full flex items-center justify-center min-w-[20px] h-5 {{ $cartCount > 0 ? 'scale-100' : 'scale-0' }}">
                    {{ $cartCount }}
                </span>
            </a>
                ...
                <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition-all duration-500 animate-fade-up animate-delay-{{ $loop->index * 50 }}"
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-4 cursor-pointer"
                            ...
                            <div class="mb-4 md:mb-0">
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    ...
                                    @if($order->order_type === 'dine_in' && $order->meja)
                                        | Meja: <span class="font-semibold">{{ $order->meja->nomor_meja }}</span>
                                    @endif
                                </p>
                            </div>
                            <div class="flex items-center space-x-4">
                        ...
                        <div class="border-t border-gray-200 pt-4 overflow-hidden transition-all duration-500 ease-in-out"
                            ...
                            <!-- Tampilkan info tipe pesanan dan meja -->
                            <div class="space-y-3">
                                <div class="mb-4 text-sm text-gray-600">
                                    <span class="font-semibold">Tipe Pesanan:</span>
                                    {{ $order->order_type === 'dine_in' ? 'Dine In' : 'Takeaway' }}
                                    @if($order->order_type === 'dine_in' && $order->meja)
                                        | <span class="font-semibold">Meja:</span> {{ $order->meja->nomor_meja }} (Kapasitas: {{ $order->meja->kapasitas }})
                                    @endif
                                </div>
                                    ...
                                    <div class="flex justify-between items-center p-3 rounded-lg hover:bg-gray-50 transition-colors duration-200 border border-gray-100 animate-fade-in animate-delay-{{ $loop->index * 50 }}">
                                        <div class="flex items-center">
                                            <div class="w-10 h-10 rounded-md overflow-hidden mr-3">
                                                <img src="{{ asset('storage/' . $item->menu->gambar) }}" alt="{{ $item->menu->nama_menu }}" class="w-full h-full object-cover">
                                            </div>
                                            <div>
                            ...
                            @if($order->status === 'pending')
                                <div class="flex justify-end space-x-3 mt-5 animate-fade-in">
                                    <form method="POST" action="{{ route('customer.orders.update-status', $order->id) }}">
                                        ...
                                        <button type="submit" class="text-sm text-red-600 hover:text-red-800 font-medium px-4 py-2 rounded-lg border border-red-200 hover:bg-red-50 transition-all duration-300 flex items-center transform hover:scale-105">
                                            <i class="fas fa-times-circle mr-2"></i>
                                            Batalkan Pesanan
                                        </button>
                                    </form>
                                    <a href="https://wa.me/08xxxxxxxxxx" target="_blank" class="text-sm bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-medium px-4 py-2 rounded-lg transition-all duration-300 flex items-center shadow-md hover:shadow-lg transform hover:scale-105">
...
{{-- @push('scripts')
<script>
    // Tidak perlu simulasi status otomatis di sisi customer
    // Jika ingin AJAX update status, gunakan kode di bawah (opsional, untuk admin)
